...
class RealtimeClientConnection {
  ...
  registerEventHandler () {
    ...
    // received note change password request
    this.socket.on('passwordPermission', this.passwordPermissionChangeEventHandler.bind(this))
  }
  ...
  async changeNotePasswordPermission (newPassword) {
    console.log('change note permission password')
    const [changedRows] = await models.Note.update({
      permission: 'password',
      password: newPassword,
      passwordVersion: Sequelize.literal('"passwordVersion" + 1')
    }, {
      where: {
        id: this.getCurrentNote().id
      }
    ...
    if (changedRows !== 1) {
      throw new Error(`updated permission failed, cannot set permission password to note ${this.getCurrentNote().id}`)
    }
  }
  ...
  notifyPasswordPermissionChanged () {
    console.log('notify password permission')
    this.realtime.io.to(this.getCurrentNote().id).emit('permission', {
      permission: 'password'
    })
    this.getCurrentNote().socks.forEach((sock) => {
      if (sock) {
        ...
        const owner = this.getCurrentNote().owner
        if (!(req.user && req.user.logged_in && req.user.id === owner)) {
          sock.emit('reload')
          setTimeout(function () {
            sock.disconnect(true)
          }, 0)
  ...
  passwordPermissionChangeEventHandler (password) {
    ...
    if (!('password' in this.getAvailablePermissions())) return

    this.changeNotePasswordPermission(password)
      .then(() => {
        note.permission = 'password'
        note.password = password
        note.passwordVersion += 1
        this.notifyPasswordPermissionChanged()
      })
      .catch(err => logger.error('update note permission password failed: ' + err))
  }
