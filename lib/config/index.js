...
// s3 use self-signed CA
if (config.customS3Endpoint.enable) {
  let signedCA
  if (fs.existsSync(config.customS3Endpoint.signedCAChain)) {
    signedCA = fs.readFileSync(config.customS3Endpoint.signedCAChain, 'utf8')
    let httpsOptions = {
    ...
    Object.assign(config.s3, {
      'endpoint': config.customS3Endpoint.endpoint,
      's3ForcePathStyle': config.customS3Endpoint.s3ForcePathStyle,
