...
exports.checkViewPasswordPermission = checkViewPasswordPermission
...
function checkPasswordInSession (session, note) {
  // use optional chaining after upgrading node.js to
  return session && session.hasPassword && session.hasPassword.some((passwordInfo) => {
    return passwordInfo.noteId === note.id && passwordInfo.passwordVersion === note.passwordVersion
  })
}

function checkViewPasswordPermission (req, note) {
  if (req.isAuthenticated() && note.ownerId === req.user.id) {
    return true
  }
  return checkPasswordInSession(req.session, note)
}
...
function findNote (req, res, callback, include) {
  ...
  models.Note.parseNoteId(id, function (err, _id) {
    ...
    models.Note.findOne({
      ...
      if (note.permission === 'password' && !checkViewPasswordPermission(req, note)) {
