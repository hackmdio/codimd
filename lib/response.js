...
exports.errorNeedPassword = errorNeedPassword
exports.errorIncorrectPassword = errorIncorrectPassword
...
exports.checkPasswordInSession = checkPasswordInSession
exports.responseCodiMD = responseCodiMD

function errorNeedPassword (res) {
  return errorPassword(res, 'This note is protected by password.')
}

function errorIncorrectPassword (res) {
  return errorPassword(res, 'Incorrect password, please try again')
}

function errorPassword (res, msg) {
  res.status('401').render('password.ejs', {
    title: '401 Unauthorized ' + msg,
    msg
  })
}
...
function checkPasswordInSession (req, note) {
  console.log('permission is password, check password in session')
  if (req.isAuthenticated() && note.ownerId === req.user.id) {
    console.log(`it is owner, doesn't need password`)
    return true
  }
  // use optional chaining after upgrading node.js to
  console.log(`req.session = ${JSON.stringify(req.session, null, 2)}`)
  console.log(`note = ${JSON.stringify(note, null, 2)}`)
  if ((req.session && req.session.hasPassword && req.session.hasPassword.some((passwordInfo) => {
    return passwordInfo.noteId === note.id && passwordInfo.passwordVersion === note.passwordVersion
  }))) {
    return true
  } else {
    return false
...
function findNote (req, res, callback, include) {
  ...
  models.Note.parseNoteId(id, function (err, _id) {
    ...
    models.Note.findOne({
      ...
      if (note.permission === 'password' && !checkPasswordInSession(req, note)) {
        return errorNeedPassword(res)
      }
