...
const { newCheckViewPermission, checkPasswordInSession, errorIncorrectPassword, errorNeedPassword, errorForbidden, responseCodiMD, errorNotFound, errorInternalError } = require('../response')
...
async function showNote (req, res) {
  ...
  if (note.permission == 'password') {
    if (!checkPasswordInSession(req, note)) {
...
async function tryNotePassword (req, res) {
  let note = await getNoteById(req.params.noteId)
  if (req.body.password === note.password) {
    req.session.hasPassword = req.session.hasPassword || [];
    // remove old version
    req.session.hasPassword = req.session.hasPassword.filter((({noteId}) => noteId != note.id))
    req.session.hasPassword.push({
      noteId: note.id,
      passwordVersion: note.passwordVersion,
    })
    res.redirect('back')
  } else {
...
exports.tryNotePassword = tryNotePassword
