...
function getTags(rawContent) {
  const tags = []
  var lines = String(rawContent).split('\n')
  for(var i = 0;i < lines.length;i++) {
    if (lines[i].includes("###### tags: ")) {
      var tagString = lines[i].replace("###### tags: ", "").split(" ")
      for(var j = 0;j < tagString.length;j++) {
        if (!tags.includes(tagString[j].replace(/`/g,"")) && tagString[j].slice(0,1) === "`" && tagString[j].slice(-1) === "`") {
          tags.push(tagString[j].replace(/`/g,""))
        }
      }
    }
  }
...
function getShareHistory (offset, keywords = "", authStatus = false, callback) {
  keywords = "%"+keywords+"%"
  if (authStatus) {
    var permission = ['freely' ,'editable', 'limited', 'locked', 'protected']
  } else {
    var permission = ['freely' ,'editable', 'locked']
  }
  models.Note.findAll({
    attributes: ['id', 'title', 'lastchangeAt', 'content'],
    where: {
      [Op.and]: [
        {
