...
function getShareHistory (offset, keywords = "", callback) {
  keywords = "%"+keywords+"%"
  models.Note.findAll({
    attributes: ['id', 'title', 'lastchangeAt'],
    where: {
      [Op.and]: [
        {
          permission: ['freely' ,'editable', 'limited', 'locked', 'protected']
        },
        {
          [Op.or]: [
            {
...
function ShareHistoryGet (req, res) {
  if (req.isAuthenticated()) {
    getShareHistory(parseInt(req.query.offset), req.query.keywords, function (err, history) {
